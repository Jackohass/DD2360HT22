...
int main(int argc, char **argv) {
  ...
  FILE *fp = fopen("data/ass4ex4OutputFloat.csv", "a");
  ...
  cudaMallocManaged(&AColIndx, nzv*sizeof(double));
  cudaMallocManaged(&ARowPtr, (dimX+1)*sizeof(double));
  
  cputimer_stop("Allocating device memory");

  //concurrentAccessQ = false;
  ...
  if (concurrentAccessQ) {
    ...
    cudaMemPrefetchAsync(A, nzv*sizeof(double), cudaCpuDeviceId, NULL);
    cudaMemPrefetchAsync(AColIndx, nzv*sizeof(double), cudaCpuDeviceId, NULL);
    cudaMemPrefetchAsync(ARowPtr, (dimX+1)*sizeof(double), cudaCpuDeviceId, NULL);
  ...
  if (concurrentAccessQ) {
    ...
    cudaMemPrefetchAsync(A, nzv*sizeof(double), device, NULL);
    cudaMemPrefetchAsync(AColIndx, nzv*sizeof(double), device, NULL);
    cudaMemPrefetchAsync(ARowPtr, (dimX+1)*sizeof(double), device, NULL);
  ...
  cusparseSetMatType(Adescriptor, CUSPARSE_MATRIX_TYPE_GENERAL);*/
  ...
  cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor,
    vecX, &zero, vecY, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize
  );
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    //@@ tmp = 1 * A * temp + 0 * tmp
    //cputimer_start();
    cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor,
      vecX, &zero, vecY, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, buffer
    );
    //cudaDeviceSynchronize();
    //totTime += cputimer_stop("Calculating")/1000000;
  ...
  double flops = 2*nzv*(((double)nsteps)/1000000000);
  double theoryFLOPS = flops/totTime;
  printf("Total time: %f s, FLOPs: %f, FLOPS: %f\n", totTime, flops, theoryFLOPS);
  //fprintf(fp, "dimX: %d, %d, %f\n", dimX, nsteps, theoryFLOPS);
  fprintf(fp, "dimX: %d, %d, %f\n", dimX, nsteps, error);
