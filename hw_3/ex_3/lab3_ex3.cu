...
{
  //@@ Insert code below to compute histogram of input using shared memory and atomics
  const int idx = threadIdx.x*3 + blockDim.x * blockIdx.x;
  if (idx >= num_elements) return;
  ...
  DataType num1 = input[idx + 1];
  DataType num2 = input[idx + 2];
  __syncthreads();
  ...
  if (s_idx == 0)
  {
    for (int i = 0; i < NUM_BINS; i++)
    {
      ...
      atomicAdd(bins + index, s_prod[index]);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  for(int i = 0; i < inputLength; i++)
  {
    if(resultRef[hostInput[i]] < 127) resultRef[hostInput[i]]++;
  }
  ...
  startTime();
  histogram_kernel3<<<block0, grid0>>>(deviceInput, deviceBins, inputLength, NUM_BINS);
  ...
  convert_kernel<<<block1, grid1>>>(deviceBins, NUM_BINS);
  cudaDeviceSynchronize();
  ...
  printf("CPU: %f ms, H->D: %f ms, D->H: %f ms, Kernel: %f ms", CPUTime, HToDTime, DToHTime, kernelTime);
  fprintf(fp, "%f, %f, %f, %f", CPUTime, HToDTime, DToHTime, kernelTime);
